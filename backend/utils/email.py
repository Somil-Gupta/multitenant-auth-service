...
def send_signup_invite_email(email: str, password: str | None = None):
    generated_token = AuthService(next(get_db())).create_refresh_token({"sub": email})
...
def send_org_invite_email(email: str, organization: str):
    ...
    <p>If it is your first time using Multitenant App, then please verify your email by link received in separate email.</p>
    """
    return send_email(subject="Verify your email to complete Signup", email_to=email, content=content)
